...
/* ═══════════════════════════════════════════════════════════════════
   SORT ANIMATION
   M11: animateSort() - .sort() and .sort(reverse=True)
   
   TIMELINE (same-dtype list):
   t=0.0   : Content cells fade out (0.3s)
   t=0.3–2.3 : Labels ("Smallest element", etc.) stagger-fade in over 2s
               Each label triggers a rising Tone.js note (C5 → … → last note)
   t=2.3   : All labels visible
   t=2.5   : Cell index 0: text fades out → yellow highlight → actual value fades in
              Then same for index 1, 2, … n-1 (each staggered ~0.2s apart)
              Simultaneously: full note sequence (C5→last) plays in 0.2s total
   t~final : All cells show sorted values. Yellow highlights fade to normal.
   ═══════════════════════════════════════════════════════════════════ */

// Track sort elements for cleanup
let activeSortElements = [];

/**
 * Cleanup sort animation elements
 */
export function cleanupSortElements() {
    activeSortElements.forEach(el => {
        try { el.remove(); } catch (e) { /* already removed */ }
    });
    activeSortElements = [];
    document.querySelectorAll('.sort-label-text').forEach(el => el.remove());
    document.querySelectorAll('.sort-overlay').forEach(el => el.remove());
}

// ─── Musical scale for sort animation (Tone.js notes) ───────────────────────
// We use a major pentatonic scale starting from C5 for a satisfying ascending feel
const SORT_NOTES = ['C5', 'D5', 'E5', 'G5', 'A5', 'C6', 'D6', 'E6', 'G6', 'A6', 'C7'];

/**
 * Get Tone.js note for index i, for a list of size n
 * Spreads notes evenly across SORT_NOTES
 */
function getSortNote(i, n) {
    const maxNoteIdx = SORT_NOTES.length - 1;
    if (n === 1) return SORT_NOTES[0];
    const noteIdx = Math.round((i / (n - 1)) * maxNoteIdx);
    return SORT_NOTES[Math.min(noteIdx, maxNoteIdx)];
}

/**
 * Play a single Tone.js note (short piano-like hit)
 */
function playSortNote(note) {
    if (typeof Tone === 'undefined') return;
    try {
        ...
        syn.volume.value = -8;
        syn.triggerAttackRelease(note, '8n');
        // Dispose after it's done
        setTimeout(() => { try { syn.dispose(); } catch (e) { } }, 1000);
    } catch (e) {
        console.warn('Sort note error:', e);
    }
}

/**
 * Play all n notes very rapidly (0.2s total), used during value-reveal phase
 */
function playSortArpeggio(n) {
    if (typeof Tone === 'undefined') return;
    const interval = n <= 1 ? 0 : 200 / (n - 1); // ms between notes
    for (let i = 0; i < n; i++) {
        const note = getSortNote(i, n);
        setTimeout(() => playSortNote(note), i * interval);
    }
}

/**
 * Get the ordinal label for position i (0-indexed), for ascending or descending sort
 * @param {number} i - 0-indexed position
 * @param {number} n - total elements
 * @param {boolean} isReverse - true for descending sort
 */
function getSortLabel(i, n, isReverse) {
    const direction = isReverse ? 'Largest' : 'Smallest';
    if (i === 0) return direction + ' element';
    if (n === 2) return (isReverse ? '2nd Largest' : '2nd Smallest') + ' element';
    const suffixes = ['th', 'st', 'nd', 'rd'];
    const pos = i + 1; // 1-indexed
    const suffix = (pos >= 11 && pos <= 13)
        ? 'th'
        : (suffixes[pos % 10] || 'th');
    return `${pos}${suffix} ${direction} element`;
}
 ...
 *
 * @param {HTMLElement} listContainer — the .list-container element
 * @param {Array<string>} originalItems — original (unsorted) values as strings
 * @param {Array<string>} sortedItems  — sorted values as strings
 * @param {boolean} isReverse          — true when .sort(reverse=True)
 * @param {Function} onComplete        — called when all animations finish
 */
export function animateSort(listContainer, originalItems, sortedItems, isReverse, onComplete) {
    const n = originalItems.length;
    activeSortElements = [];
    ...
    const rows = Array.from(listContainer.querySelectorAll('.list-row'));
    const contentCells = rows.map(r => r.querySelector('.list-content-cell'));

    const tl = gsap.timeline({
        onComplete: () => {
            // Final cleanup — remove any leftover label spans
            contentCells.forEach((cell, idx) => {
                // Remove label spans
                cell.querySelectorAll('.sort-label-text').forEach(el => el.remove());
                // Ensure correct final value shown
                const valueEl = cell.querySelector('.list-value');
                const displayVal = sortedItems[idx];
    ...
    // ════════════════════════════════════════════════════════════════════
    // PHASE 1 (t=0): Fade out all content cell text
    // ════════════════════════════════════════════════════════════════════
    ...
    // ════════════════════════════════════════════════════════════════════
    // PHASE 2 (t=0.3 – 2.3): Stagger label placeholders + notes over 2s
    // ════════════════════════════════════════════════════════════════════
    const labelPhaseStart = fadeDuration;
    const labelPhaseDuration = 2.0;
    const labelStagger = n <= 1 ? 0 : labelPhaseDuration / (n - 1);

    contentCells.forEach((cell, i) => {
        const labelStart = labelPhaseStart + i * labelStagger;
        const labelText = getSortLabel(i, n, isReverse);
        const note = getSortNote(i, n);

        tl.call(() => {
            // Clear previous text content (hide original value)
            const valueEl = cell.querySelector('.list-value');
            ...
            activeSortElements.push(span);

            // Fade in the label
            gsap.to(span, { opacity: 1, duration: 0.25, ease: 'power1.out' });

            // Fade in the cell itself
            gsap.to(cell, { opacity: 1, duration: 0.25 });
    ...
    // ════════════════════════════════════════════════════════════════════
    // PHASE 3 (t=2.5+): Replace labels with actual sorted values
    //   - Each cell reveal is staggered by 0.2s
    //   - Yellow highlight → black text fades in
    //   - Arpeggio plays across all cells simultaneously
    // ════════════════════════════════════════════════════════════════════
    const revealPhaseStart = labelPhaseStart + labelPhaseDuration + 0.2;
    const perCellRevealDuration = 0.18; // each cell takes 0.18s

    // Play the full arpeggio at reveal start
    tl.call(() => {
        playSortArpeggio(n);
    }, null, revealPhaseStart);

    contentCells.forEach((cell, i) => {
        const revealStart = revealPhaseStart + i * 0.2;

        tl.call(() => {
            // Remove label span
            cell.querySelectorAll('.sort-label-text').forEach(el => {
                gsap.to(el, { opacity: 0, duration: 0.1, onComplete: () => el.remove() });
            });

            // Restore/set actual sorted value on the underlying element
            const valueEl = cell.querySelector('.list-value');
            const displayVal = sortedItems[i];
            if (valueEl) {
                valueEl.textContent = displayVal;
                valueEl.style.visibility = 'visible';
                valueEl.style.opacity = '0';
                valueEl.style.color = '#111827'; // near-black
        ...
        tl.call(() => {
            const valueEl = cell.querySelector('.list-value');
            if (valueEl) {
                gsap.to(valueEl, { opacity: 1, duration: perCellRevealDuration, ease: 'power1.out' });
            } else {
                gsap.to(cell, { opacity: 1, duration: perCellRevealDuration, ease: 'power1.out' });
            }
        }, null, revealStart + 0.08);

        // Fade yellow highlight back to normal after brief pause
        tl.to(cell, {
            backgroundColor: '',
            duration: 0.5,
            ease: 'power1.out'
...
/* ═══════════════════════════════════════════════════════════════════
   M11b: animateSortMismatch() - TypeError when list contains mixed types
   
   TIMELINE:
   t=0.0 : "Comparing elements..." indicator appears beside the list  
   t=0.5 : Scan through rows highlighting each cell amber briefly
   t=~n*0.2+0.5 : Two mismatching cells flash red simultaneously + shake
   t=+0.5 : Error chord (Tone.js diminished)
   t=+0.8 : TypeError pill flies from the list to the output panel
   t=+1.5 : Indicator fades out
   ═══════════════════════════════════════════════════════════════════ */

/**
 * M11b: animateSortMismatch()
 *
 * @param {HTMLElement} listContainer — the .list-container element
 * @param {HTMLElement} outputPanel   — the #output element
 * @param {Function}    onComplete    — called when animation finishes
 */
export function animateSortMismatch(listContainer, outputPanel, onComplete) {
    const rows = Array.from(listContainer.querySelectorAll('.list-row'));
    const n = rows.length;

    // ─── Create "Comparing elements..." indicator ─────────────────────────────
    const indicator = document.createElement('div');
    indicator.className = 'sort-overlay';
    ...
    indicator.innerHTML = `
        <div style="margin-bottom:4px">sort()</div>
        <div style="font-size:11px;font-style:italic;color:#c7d2fe" class="sort-status-text">Comparing elements...</div>
    `;
    document.body.appendChild(indicator);
    activeSortElements.push(indicator);
    ...
    // Position indicator to the left of the list
    tl.call(() => {
        const rect = listContainer.getBoundingClientRect();
        indicator.style.left = `${rect.left - 210}px`;
        indicator.style.top = `${rect.top + rect.height / 2 - 35}px`;
    }, null, 0);

    // Fade indicator in
    tl.to(indicator, { opacity: 1, scale: 1, duration: 0.4, ease: 'back.out(1.7)' }, 0);

    // ─── Scan: amber flash through each row ───────────────────────────────────
    let currentTime = 0.5;
    const scanPerRow = Math.min(0.2, 1.5 / Math.max(n, 1));

    rows.forEach((row, i) => {
        tl.call(() => {
            row.style.backgroundColor = '#fef3c7'; // amber-50
            playScanTick();
        }, null, currentTime);
        tl.call(() => {
            row.style.backgroundColor = '';
        }, null, currentTime + scanPerRow * 0.8);
        currentTime += scanPerRow;
    });

    // ─── Find two "conflicting" cells (first + second, or first + last) ───────
    // Visually highlight the pair that looks like they conflict
    const badIdxA = 0;
    const badIdxB = Math.min(1, n - 1);
    const badRowA = rows[badIdxA];
    const badRowB = rows[badIdxB];

    const conflictStart = currentTime;

    // Red flash on both cells
    tl.call(() => {
        [badRowA, badRowB].forEach(r => {
            if (r) r.style.backgroundColor = '#fca5a5'; // red-300
        });
    }, null, conflictStart);
    ...
    tl.call(() => {
        playErrorChord();
    }, null, conflictStart + 0.1);

    // Shake the container
    tl.to(listContainer, {
        x: '+=8', yoyo: true, repeat: 5, duration: 0.07
    }, conflictStart + 0.1);

    // Update status text
    tl.call(() => {
        const statusEl = indicator.querySelector('.sort-status-text');
        if (statusEl) {
            statusEl.textContent = 'Type mismatch!';
            statusEl.style.color = '#fca5a5';
        }
    }, null, conflictStart + 0.4);

    // ─── TypeError pill flies to output ──────────────────────────────────────
    tl.call(() => {
        const indicatorRect = indicator.getBoundingClientRect();
        const startX = indicatorRect.right;
        const startY = indicatorRect.top + indicatorRect.height / 2;

        const outputRect = outputPanel.getBoundingClientRect();
        const targetX = outputRect.left + 60;
        const targetY = outputRect.top + 20;

        const pill = document.createElement('div');
        pill.textContent = "TypeError: '<' not supported";
        ...
        gsap.to(pill, {
            ...
            ease: 'power2.out',
            onComplete: () => {
                // Write to output panel as well
                const line = document.createElement('div');
                line.className = 'output-line';
                line.style.color = '#ef4444';
                line.textContent = "TypeError: '<' not supported between instances of 'str' and 'int'";
                outputPanel.appendChild(line);
                gsap.to(pill, { opacity: 0, duration: 0.3, onComplete: () => pill.remove() });
            }
        });
    }, null, conflictStart + 0.8);
